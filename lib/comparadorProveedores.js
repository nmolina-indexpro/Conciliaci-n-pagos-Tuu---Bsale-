...
export function parsearLineaTexto(lineaOriginal) {
  ...
  //
  // Se separa SOLO por tab cuando hay al menos uno en la línea (pegado real
  // desde Excel) -- si además se tratara "2+ espacios" como separador, una
  // celda de descripción con doble espacio antes del modelo de panel (ej.
  // "S/BRACKET  NV140FHM-N48") se cortaría a la mitad y el modelo quedaría
  // fuera de la descripción, justo el dato que más importa para no
  // confundir dos pantallas distintas. Sin tabs (texto alineado a mano) sí
  // se usa "2+ espacios" como única señal disponible.
  const campos = linea.includes('\t')
    ? linea.split('\t').map(c => c.trim()).filter(Boolean)
    : linea.split(/ {2,}/).map(c => c.trim()).filter(Boolean);

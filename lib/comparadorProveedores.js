...
// "7.4x5" y "7.4x5.0" son el mismo conector -- compara ambos lados de la
// "x" como número, no como texto (ver evaluarEquivalencia).
function conectorEquivalente(a, b) {
  const [a1, a2] = String(a).split('x').map(Number);
  const [b1, b2] = String(b).split('x').map(Number);
  return a1 === b1 && a2 === b2;
}
...
export function evaluarEquivalencia(especificacionesPedido, especificacionesCotizacion) {
  ...
  for (const campo of campos) {
    ...
    // "conector" queda guardado como texto "AxB" (ej. "7.4x5"), no como
    // número -- "7.4x5" y "7.4x5.0" son el MISMO conector pero texto
    // distinto (bug real: un lado escribe el milímetro con decimal y el
    // otro no), así que acá se compara valor a valor, no como string.
    const coincide = campo === 'conector' ? conectorEquivalente(valorPedido, valorCotizacion) : valorCotizacion === valorPedido;
    if (!coincide) {
...
// Tokeniza para la comparación de similitud por texto (Jaccard) -- distinto
// de normalizarTexto solo/a secas, que se deja intacta para las regex de
// extraerEspecificaciones (esas SÍ necesitan el guion literal para
// reconocer un modelo de panel como "NV140FHM-N48"). Bug real detectado:
// un mismo dato técnico escrito con guion pegado de un lado ("19.5V-3.34A",
// "PA-12") y separado por espacios del otro ("19.5V 3.34A", "PA12") no
// generaba NINGÚN token en común, así que la similitud de texto caía bajo
// el umbral de 0.2 y la línea quedaba "sin emparejar" aunque fuera
// evidentemente el mismo producto. Por cada palabra se agregan hasta 3
// variantes al set de tokens: tal cual, sin el guion (pegada, "pa-12" ->
// "pa12"), y partida en el guion (separada, "19.5v-3.34a" -> "19.5v" +
// "3.34a") -- cubre ambos casos sin tocar normalizarTexto.
function tokenizarParaSimilitud(texto) {
  const tokens = new Set();
  for (const palabra of normalizarTexto(texto).split(' ').filter(Boolean)) {
    if (palabra.length > 2) tokens.add(palabra);
    if (palabra.includes('-')) {
      const pegada = palabra.replace(/-/g, '');
      if (pegada.length > 2) tokens.add(pegada);
      for (const parte of palabra.split('-')) {
        if (parte.length > 2) tokens.add(parte);
      }
    }
  }
  return tokens;
...
export function emparejarLineaCotizacion(descripcionProveedor, items) {
  const tokensProveedor = tokenizarParaSimilitud(descripcionProveedor);
  const especificacionesProveedor = extraerEspecificaciones(descripcionProveedor);

  let mejor = null;
  for (const item of items) {
    const tokensPedido = tokenizarParaSimilitud(item.nombre);
    ...
    // item.especificaciones llega como "{}" (JSONB vacío, ni null ni
    // undefined) para todo ítem creado desde el formulario rápido de la UI,
    // que hoy no pide specs por separado -- un "{}" es truthy en JS, así
    // que había que comprobar que tenga ALGUNA clave antes de usarlo, o el
    // fallback a extraerlas del nombre nunca se activaba.
    const tieneEspecificacionesGuardadas = item.especificaciones && Object.keys(item.especificaciones).length > 0;
    const especificacionesPedido = tieneEspecificacionesGuardadas ? item.especificaciones : extraerEspecificaciones(item.nombre);
...
// Fila separadora de una tabla markdown (ej. "| --- | :---: | ---: |") --
// no es un producto, hay que descartarla antes de intentar interpretarla
// como una línea de cotización (si no, "---"/"-" se cuela como si fuera un
// código o una descripción real).
function esFilaSeparadorMarkdown(linea) {
  return /^\|?\s*:?-{2,}:?\s*(\|\s*:?-{2,}:?\s*)+\|?$/.test(linea);
}

// Divide una línea en campos según el separador que realmente use: tabla
// markdown pegada (pipes "|", ej. copiado de un chat/documento con formato),
// tab (pegado real desde Excel), o "2+ espacios" (texto alineado a mano).
// Se prueban en ese orden porque una fila con pipes también puede tener
// tabs o espacios dobles alrededor de cada "|" y esos NO deben usarse como
// separador aparte (partirían cada celda de nuevo). Los "**negrita**" de
// markdown se sacan de cada celda (ej. "**21010506N**" -> "21010506N")
// para que el código/precio se reconozcan igual que en texto plano.
function dividirCampos(linea) {
  const limpiar = c => c.trim().replace(/^\*+|\*+$/g, '').trim();
  if (/\|/.test(linea) && linea.split('|').length > 2) {
    return linea.split('|').map(limpiar).filter(Boolean);
  }
  if (linea.includes('\t')) {
    return linea.split('\t').map(limpiar).filter(Boolean);
  }
  return linea.split(/ {2,}/).map(limpiar).filter(Boolean);
}

export function parsearLineaTexto(lineaOriginal) {
  const linea = String(lineaOriginal || '').trim();
  if (!linea || esFilaSeparadorMarkdown(linea)) return null;

  // Pegado desde Excel (tabs), tabla markdown (pipes) o alineado en
  // columnas con espacios múltiples -> se separa en campos; si no, se trata
  // como una sola frase libre. La extracción de cantidad y código SOLO se
  // intenta en filas tabulares -- en texto libre un número o token suelto
  // casi siempre es parte de una especificación (voltaje, amperaje,
  // modelo), no la cantidad ni el código, así que ahí se prefiere dejarlos
  // vacíos antes que adivinar mal (ej. tomar "19V" como código, o el "7" de
  // "7.5A" como cantidad).
  //
  // Nota sobre espacios dobles: una celda de descripción con doble espacio
  // antes del modelo de panel (ej. "S/BRACKET  NV140FHM-N48") se cortaría a
  // la mitad si se tratara "2+ espacios" como separador junto con otra
  // señal -- por eso dividirCampos prueba pipe/tab primero y solo cae a
  // espacios cuando no hay ninguna de las otras dos señales.
  ...
  // al final de la fila. Excepción: una cotización/factura con columnas
  // "Precio" y "Total" (Total = Precio × Cantidad) también termina en un
  // número -- si el último número calza EXACTO con el anterior multiplicado
  // por la cantidad detectada, es el total de la línea, no el precio
  // unitario, así que se prefiere el anterior.
  const coincidenciasPrecio = [...linea.matchAll(/\$?\s*(\d{1,3}(?:\.\d{3})+|\d{4,})\b/g)].map(m => parseInt(m[1].replace(/\./g, ''), 10));
  let precio_neto = coincidenciasPrecio.length > 0 ? coincidenciasPrecio[coincidenciasPrecio.length - 1] : null;
  if (coincidenciasPrecio.length > 1 && cantidad_disponible > 1) {
    const anterior = coincidenciasPrecio[coincidenciasPrecio.length - 2];
    if (Math.round(anterior * cantidad_disponible) === precio_neto) precio_neto = anterior;
  }
...
// Si viene de pegar una tabla (Excel, WhatsApp con columnas), la primera
// línea suele ser el encabezado ("SKU", "Producto", "Precio"...) -- se
// descarta cuando ninguno de sus campos parece un dato real (ni precio
// tipo miles, ni un número puro), para que no quede como una línea
// "sin emparejar" más en la comparativa.
function esProbableEncabezado(linea) {
  if (!/\t| {2,}|\|/.test(linea)) return false;
  const campos = dividirCampos(linea);
  return !campos.some(c => /^\$?\s*\d{1,3}(?:\.\d{3})*$/.test(c) || /^\d{4,}$/.test(c));
}

export function parsearTextoCotizacion(texto) {
  // Las filas separadoras de una tabla markdown (ver esFilaSeparadorMarkdown)
  // se descartan acá, ANTES de decidir cuál es el encabezado -- si no, la
  // fila "| --- | --- |" (que va justo después del encabezado real) se
  // toma por error como la primera línea de datos.
  const lineas = String(texto || '').split('\n').map(l => l.trim()).filter(l => l && !esFilaSeparadorMarkdown(l));
  const lineasDatos = (lineas.length > 0 && esProbableEncabezado(lineas[0])) ? lineas.slice(1) : lineas;
  return lineasDatos.map(l => parsearLineaTexto(l)).filter(Boolean);
}

...
export function firmarSesion(payload, expiraEnMs) {
  const secret = process.env.AUTH_SECRET;
  if (!secret) throw new Error('AUTH_SECRET no está configurada en el servidor');
  let exp = Date.now() + DIAS_SESION * 24 * 60 * 60 * 1000;
  // Si la cuenta tiene su propia fecha de expiración (cuentas temporales) y
  // esa fecha es más cercana que el vencimiento normal de sesión, la cookie
  // hereda esa fecha -> la sesión muere exactamente cuando debe morir la
  // cuenta, sin depender de que alguien la cierre a mano.
  if (typeof expiraEnMs === 'number' && expiraEnMs < exp) exp = expiraEnMs;
  const cuerpo = { ...payload, exp };

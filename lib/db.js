...
export async function asegurarTablaUsuarios(sql) {
  ...
  // Recuperación de contraseña por correo (ver api/negocio.js ->
  // manejarAuthRecuperarPassword / manejarAuthResetearPassword). Se guarda
  // el HASH del token (sha256), no el token crudo -- el mismo criterio que
  // password_hash: si la base se filtra, no queda un token usable directo.
  // reset_token_expira: vence a los 30 min, después de eso el link ya no
  // sirve. reset_token_creado_en: separado de la expiración para poder
  // limitar reenvíos en ráfaga (no generar/mandar uno nuevo si ya se pidió
  // hace menos de unos minutos) sin depender de si el anterior ya venció.
  await sql`ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS reset_token_hash TEXT;`;
  await sql`ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS reset_token_expira TIMESTAMPTZ;`;
  await sql`ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS reset_token_creado_en TIMESTAMPTZ;`;
}
...
export async function asegurarTablaCotizaciones(sql) {
  ...
  // Fecha de emisión de la factura/boleta asociada -- para poder calcular
  // "cuánto tardó de cotización a factura" (ver manejarSyncCotizaciones,
  // Fase 2, y la tabla de Vendedores en oportunidades-comerciales.html).
  // Las cotizaciones vinculadas ANTES de este cambio quedan sin esta
  // fecha hasta que se vuelvan a tocar (no afecta su vínculo, solo este
  // cálculo puntual).
  await sql`ALTER TABLE bsale_cotizaciones ADD COLUMN IF NOT EXISTS documento_asociado_fecha DATE;`;
  ...
  `;
  // Cursor de la Fase 2 (por cliente_id, ver manejarSyncCotizaciones): sin
  // esto, la query de clientes pendientes se re-consultaba completa en cada
  // llamada sin ORDER BY, y como Postgres devuelve el mismo orden mientras
  // los datos no cambian, los clientes que quedaban después del corte por
  // presupuesto nunca llegaban a procesarse (starvation) -- confirmado en
  // vivo el 2026-09-08: dos sincronizaciones seguidas procesaron los mismos
  // 54 clientes y encontraron 0 vínculos nuevos ambas veces.
  await sql`ALTER TABLE bsale_cotizaciones_sync_estado ADD COLUMN IF NOT EXISTS cliente_cursor INTEGER NOT NULL DEFAULT 0;`;
  await sql`INSERT INTO bsale_cotizaciones_sync_estado (id) VALUES (1) ON CONFLICT (id) DO NOTHING;`;
}

// Historial de los cambios de estado de una cotización (Sin contactar ->
// Contactado -> Facturada, etc.) -- antes el UPDATE de bsale_cotizaciones.
// estado simplemente pisaba el valor anterior, sin dejar rastro de por
// dónde pasó ni cuándo. Pedido del usuario para el panel de detalle de
// cliente en Oportunidades Comerciales ("verificar los cambios de estado
// que ha tenido"). Solo se agrega (nunca se edita/borra), igual que
// comentarios_log.
export async function asegurarTablaCotizacionesHistorialEstado(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS bsale_cotizaciones_historial_estado (
      id SERIAL PRIMARY KEY,
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS bsale_cotizaciones_historial_estado_idx ON bsale_cotizaciones_historial_estado (cotizacion_id, creado_en);`;
}
...
export async function asegurarTablaAnalisis(sql) {
  ...
  // Folio, tipo y link público del documento -- se agregó para poder
  // vincular directo una orden de Compra Ágil con la factura/boleta real
  // que la facturó (ver vincularOrdenesCompraAgil en api/negocio.js), sin
  // tener que pasar por una cotización de Bsale que puede no existir (las
  // compras de Mercado Público casi siempre se facturan directo, sin
  // pasar por el flujo de cotización interno). Las filas sincronizadas
  // ANTES de este cambio quedan con estos campos en NULL hasta la próxima
  // sincronización de Análisis (no rompe nada, solo esas filas viejas no
  // sirven para este cruce hasta que se reprocesen).
  await sql`ALTER TABLE analisis_compras ADD COLUMN IF NOT EXISTS numero TEXT;`;
  await sql`ALTER TABLE analisis_compras ADD COLUMN IF NOT EXISTS tipo_documento TEXT;`;
  await sql`ALTER TABLE analisis_compras ADD COLUMN IF NOT EXISTS url TEXT;`;
...
// Historial de comentarios, compartido por todas las páginas que tengan
// esta funcionalidad (hoy: "Productos estancados" y "Clientes recurrentes"
// en Análisis) -- pedido del usuario: un comentario nuevo YA NO reemplaza
// al anterior, se agrega a una lista, y cada uno queda con quién lo
// escribió y cuándo. "contexto" separa las distintas páginas/funciones
// que usan esta misma tabla ('ventas_sku', 'clientes_recurrentes', y las
// que se agreguen después) y "entidad_id" es el identificador de esa
// página (el SKU, el id de cliente, etc.), siempre como texto.
export async function asegurarTablaComentariosLog(sql) {
  await sql`
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS comentarios_log_entidad_idx ON comentarios_log (contexto, entidad_id, creado_en DESC);`;
  // Editar un comentario propio (pedido del usuario: solo quien lo
  // escribió puede corregirlo) actualiza el texto en el mismo lugar en
  // vez de agregar una entrada nueva -- "editado_en" queda como marca de
  // que ese comentario cambió después de creado, sin guardar un historial
  // de versiones (no se pidió eso, solo poder corregir un error de tipeo
  // o actualizar el dato).
  await sql`ALTER TABLE comentarios_log ADD COLUMN IF NOT EXISTS editado_en TIMESTAMPTZ;`;
}

// Traspaso único del comentario suelto que ya existía en
// ventas_sku_comentarios (de antes de tener historial) al log -- así no se
// pierde lo que ya se había escrito. Se puede llamar en cada request sin
// problema: el NOT EXISTS hace que después de la primera vez no inserte
// nada más (no hay una forma de "correr una migración una sola vez" en
// este proyecto, ver nota de asegurar* en CLAUDE.md).
export async function migrarComentariosVentasSkuLegacy(sql) {
  await sql`
    INSERT INTO comentarios_log (contexto, entidad_id, comentario, autor, creado_en)
    SELECT 'ventas_sku', sku, comentario, actualizado_por, actualizado_en
    FROM ventas_sku_comentarios
    WHERE comentario IS NOT NULL AND comentario <> ''
      AND NOT EXISTS (
        SELECT 1 FROM comentarios_log cl WHERE cl.contexto = 'ventas_sku' AND cl.entidad_id = ventas_sku_comentarios.sku
      );
  `;
}

// Mismo traspaso, para el comentario suelto de analisis_clientes_estado.
export async function migrarComentariosClientesLegacy(sql) {
  await sql`
    INSERT INTO comentarios_log (contexto, entidad_id, comentario, autor, creado_en)
    SELECT 'clientes_recurrentes', cliente_id::text, comentario, actualizado_por, actualizado_en
    FROM analisis_clientes_estado
    WHERE comentario IS NOT NULL AND comentario <> ''
      AND NOT EXISTS (
        SELECT 1 FROM comentarios_log cl WHERE cl.contexto = 'clientes_recurrentes' AND cl.entidad_id = analisis_clientes_estado.cliente_id::text
      );
  `;
}
...
export async function asegurarTablaServiciosMensual(sql) {
  ...
  // Ranking por técnico y clientes recurrentes de Servicio Técnico (pedido
  // del usuario) -- el documento ya trae user/client, solo faltaba pedirlo
  // con expand y guardarlo. Como es un ALTER TABLE sobre una tabla que ya
  // tenía filas, las ventas sincronizadas ANTES de este cambio quedan con
  // estas columnas en NULL hasta la próxima sincronización completa (el
  // botón "Sincronizar" ya recorre los 365 días completos de nuevo cada
  // vez que corre una pasada completa, así que se autocompletan solas).
  await sql`ALTER TABLE bsale_servicios_ventas ADD COLUMN IF NOT EXISTS vendedor_id INTEGER;`;
  await sql`ALTER TABLE bsale_servicios_ventas ADD COLUMN IF NOT EXISTS vendedor_nombre TEXT;`;
  await sql`ALTER TABLE bsale_servicios_ventas ADD COLUMN IF NOT EXISTS cliente_id INTEGER;`;
  await sql`ALTER TABLE bsale_servicios_ventas ADD COLUMN IF NOT EXISTS cliente_nombre TEXT;`;
  await sql`
...
// Panel "IndexScale" en Oportunidades Comerciales -- pedido del usuario:
// leads para cold email ofreciendo diseño web / optimización de e-commerce
// (indexscale.cl), tomados de empresas que YA son clientes de Bsale (tienen
// razón social + correo) -- no de un archivo externo como Indexpro, así que
// no hace falta una fase de "buscar RUT en Bsale" ni de historial de
// compras: el vínculo con bsale_clientes_puntos ya existe desde el día uno
// (bsale_cliente_id). Ver manejarIndexscaleOportunidades en api/negocio.js
// para la re-siembra (hasta 1000 filas, en cada request, igual criterio
// que indexpro_oportunidades).
export async function asegurarTablaIndexscale(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS indexscale_oportunidades (
      id SERIAL PRIMARY KEY,
      bsale_cliente_id INTEGER UNIQUE NOT NULL,
      segmento TEXT NOT NULL DEFAULT 'sin_sitio',
      empresa TEXT,
      cliente_nombre TEXT,
      rut TEXT,
      telefono TEXT,
      ...
      puntos INTEGER,
      estado TEXT NOT NULL DEFAULT 'sin_contactar',
      presentacion_enviada_en TIMESTAMPTZ,
      sitio_verificado_en TIMESTAMPTZ,
      actualizado_por TEXT,
      actualizado_en TIMESTAMPTZ DEFAULT now(),
      created_at TIMESTAMPTZ DEFAULT now()
    );
  `;
  // 'sin_sitio' (correo personal -> candidato a crear un e-commerce nuevo) vs
  // 'potenciar' (correo con dominio propio -> candidato a optimizar el que
  // ya tiene). Columna agregada después de la primera versión de esta tabla.
  await sql`ALTER TABLE indexscale_oportunidades ADD COLUMN IF NOT EXISTS segmento TEXT NOT NULL DEFAULT 'sin_sitio';`;
  // NULL = todavía no se comprobó si el dominio realmente tiene un sitio con
  // contenido (ver manejarIndexscaleVerificarSitios en api/negocio.js) -- un
  // correo con dominio propio no garantiza que haya un e-commerce de verdad
  // detrás, puede estar caído, parkeado o ser solo una landing con un logo.
  await sql`ALTER TABLE indexscale_oportunidades ADD COLUMN IF NOT EXISTS sitio_verificado_en TIMESTAMPTZ;`;
  // La verificación de sitios (manejarIndexscaleVerificarSitios) filtra por
  // estas dos columnas en cada tanda -- con miles de filas, sin índice esa
  // consulta se vuelve más lenta a medida que crece la tabla, comiéndose
  // presupuesto de tiempo que se necesita para las peticiones HTTP en sí.
  await sql`CREATE INDEX IF NOT EXISTS idx_indexscale_segmento_verificado ON indexscale_oportunidades (segmento, sitio_verificado_en);`;
  // Marca la fila fija de prueba (empresa "Daxis", ver
  // manejarIndexscaleOportunidades) para poder excluirla de la
  // reclasificación automática por dominio de correo y de la limpieza de
  // RUT/municipalidades -- es una fila ficticia para testear el sistema,
  // no debe moverse ni desaparecer sola.
  await sql`ALTER TABLE indexscale_oportunidades ADD COLUMN IF NOT EXISTS es_prueba BOOLEAN NOT NULL DEFAULT false;`;
  // Seguimiento de apertura del primer correo (píxel de 1x1, ver
  // manejarIndexscalePixel en api/negocio.js) y del segundo correo de
  // recontactación, que solo se habilita si se detectó la apertura del
  // primero. pixel_token es un valor aleatorio por fila para que el
  // píxel no se pueda spoofear pidiendo ids consecutivos a mano.
  await sql`ALTER TABLE indexscale_oportunidades ADD COLUMN IF NOT EXISTS presentacion_abierta_en TIMESTAMPTZ;`;
  await sql`ALTER TABLE indexscale_oportunidades ADD COLUMN IF NOT EXISTS pixel_token TEXT;`;
  await sql`ALTER TABLE indexscale_oportunidades ADD COLUMN IF NOT EXISTS segundo_correo_enviado_en TIMESTAMPTZ;`;
  // Mismo patrón que indexpro_excluidos: un lead borrado a mano no debe
  // volver a aparecer la próxima vez que se re-siembre la lista.
  await sql`
    CREATE TABLE IF NOT EXISTS indexscale_excluidos (
      bsale_cliente_id INTEGER PRIMARY KEY,
      excluido_por TEXT,
      excluido_en TIMESTAMPTZ DEFAULT now()
    );
  `;
...
export async function asegurarTablaWhatsapp(sql) {
  ...
  // utm_source/medium/campaign SUELTOS (no solo la etiqueta combinada de
  // fuente_titulo) -- para poder agrupar por plataforma real (Google Ads,
  // etc.) más adelante sin tener que parsear el string combinado. Quedan
  // NULL en fuente_tipo='boton_sitio' (el botón de WhatsApp del sitio hoy
  // manda el título de la página, no parámetros de campaña de verdad -- ver
  // extraerUtmDeTexto) y en fuente_tipo='ad' (Meta ya tiene su propio
  // ctwa_clid en fuente_id, no usa estas columnas).
  await sql`ALTER TABLE whatsapp_conversaciones ADD COLUMN IF NOT EXISTS fuente_utm_source TEXT;`;
  await sql`ALTER TABLE whatsapp_conversaciones ADD COLUMN IF NOT EXISTS fuente_utm_medium TEXT;`;
  await sql`ALTER TABLE whatsapp_conversaciones ADD COLUMN IF NOT EXISTS fuente_utm_campaign TEXT;`;
  // Origen REAL según el tracking propio de Shopify (customerJourneySummary
  // del pedido) -- distinto de todo lo de arriba, que es sobre el CLIC en
  // WhatsApp. Esto es sobre cómo el cliente llegó al sitio ANTES de
  // comprar, según Shopify (confirmado con datos reales: Google Shopping,
  // campaña "SHOPPING-CARG-NOTE-V2"). Solo se llena cuando la venta se
  // vinculó a un pedido de Shopify (ver buscarVentaShopifyPorTelefono,
  // tipo='Pedido Shopify') -- las ventas que se conciliaron directo en
  // Bsale (venta en tienda física, o ya sincronizada) no tienen esto,
  // Bsale no lo registra.
  await sql`ALTER TABLE whatsapp_conversaciones ADD COLUMN IF NOT EXISTS shopify_journey_fuente TEXT;`;
  await sql`ALTER TABLE whatsapp_conversaciones ADD COLUMN IF NOT EXISTS shopify_journey_medio TEXT;`;
  await sql`ALTER TABLE whatsapp_conversaciones ADD COLUMN IF NOT EXISTS shopify_journey_campana TEXT;`;
  await sql`ALTER TABLE whatsapp_conversaciones ADD COLUMN IF NOT EXISTS shopify_journey_landing_page TEXT;`;
...
// Órdenes de compra de Mercado Público / Compra Ágil (RUT 76.200.548-4,
// CodigoEmpresa 1295270 -- ver api/negocio.js) recibidas por IndexPro,
// cacheadas en Postgres -- la API pública de Mercado Público solo permite
// consultar "órdenes de compra por día" (no hay filtro por rango de fechas
// ni por proveedor+período), y tiene un rate limit agresivo (~1 solicitud
// cada 1.3s antes de responder 429), así que recorrer ~12-13 meses en vivo
// en cada carga de la página no es viable -> se sincroniza en tandas
// resumibles (mismo patrón que bsale_cotizaciones/analisis_compras, ver
// manejarSyncCompraAgil en api/negocio.js) y la página lee este cache.
// Una fila por orden de compra (código de OC único en todo Mercado
// Público) -- reprocesar el mismo día en una pasada posterior pisa la fila
// (ON CONFLICT DO UPDATE) en vez de duplicar, y de paso refresca el estado
// si una OC pasó de "Enviada" a "Aceptada"/"Recepción Conforme"/"Cancelada"
// entre una pasada y la siguiente.
export async function asegurarTablaCompraAgil(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS compra_agil_ordenes (
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_compra_agil_ordenes_fecha ON compra_agil_ordenes (fecha_envio);`;

  // Estado de la sincronización resumible: offset_actual recorre hacia
  // atrás un total de dias_totales días (hoy - offset), fecha por fecha.
  // Al llegar al final se marca ultima_pasada_completa_en y offset vuelve
  // a 0 -> el botón "Sincronizar ahora" (o el cron) simplemente se puede
  // volver a apretar más adelante para refrescar estados recientes y
  // extender la ventana un día más; no hace falta una fase separada de
  // "solo lo nuevo".
  await sql`
    CREATE TABLE IF NOT EXISTS compra_agil_sync_estado (
      id INTEGER PRIMARY KEY DEFAULT 1,
      offset_actual INTEGER NOT NULL DEFAULT 0,
      dias_totales INTEGER NOT NULL DEFAULT 400,
      ultima_pasada_completa_en TIMESTAMPTZ,
      actualizado_en TIMESTAMPTZ DEFAULT now()
    );
  `;
  await sql`INSERT INTO compra_agil_sync_estado (id) VALUES (1) ON CONFLICT (id) DO NOTHING;`;
  // Vínculo con la cotización de Bsale que corresponde a esta OC (si se
  // encontró una) -- ver vincularOrdenesCompraAgil en api/negocio.js. La
  // factura/boleta asociada NO se duplica acá: se lee encadenada desde
  // bsale_cotizaciones.documento_asociado_* (esa tabla ya la tiene, si la
  // cotización llegó a facturarse).
  await sql`ALTER TABLE compra_agil_ordenes ADD COLUMN IF NOT EXISTS cotizacion_vinculada_id INTEGER;`;
  // Vínculo DIRECTO con la factura/boleta real (analisis_compras) --
  // aparte del de arriba, porque la mayoría de las compras de Mercado
  // Público se facturan directo sin pasar nunca por una cotización de
  // Bsale (la "cotización" ya ocurrió en el portal de Mercado Público).
  // Si algún día se encuentran los dos vínculos para la misma OC, se
  // prefiere el encadenado por cotización al mostrar (más específico:
  // significa que SÍ hubo una cotización de por medio).
  await sql`ALTER TABLE compra_agil_ordenes ADD COLUMN IF NOT EXISTS factura_vinculada_id INTEGER;`;
  // Nombre de la persona de contacto del organismo comprador (Comprador.
  // NombreContacto en la API de Mercado Público) -- resultó ser la clave
  // real para vincular: al facturar en Bsale, IndexPro registra como
  // "cliente" al CONTACTO que hizo el pedido (ej. "Cinthia Brante
  // Tobar"), no al organismo ("I MUNICIPALIDAD DE OLMUE"), así que
  // comparar solo contra el nombre del organismo nunca encontraba nada
  // para casi ninguna OC real. Ver vincularOrdenesCompraAgil en
  // api/negocio.js.
  await sql`ALTER TABLE compra_agil_ordenes ADD COLUMN IF NOT EXISTS contacto_nombre TEXT;`;
}

// Exclusión permanente de un SKU de la lista "Comercial Dos Mundos (DM)"
// en Compras -- pedido del usuario: un producto que se detecta ahí por
// error (ej. una recepción vieja mal identificada) no debe volver a
// aparecer aunque se presione "Analizar" de nuevo. Tabla propia (no
// ventas_sku_excluidos) porque es un concepto distinto: esa otra tabla es
// sobre tendencia de ventas (Análisis), esta es sobre "esto no es
// realmente un producto de DM" (Compras) -- mismo criterio de una tabla
// dedicada por lista que ya usan analisis_excluidos/indexpro_excluidos.
export async function asegurarTablaComprasDMExcluidos(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS compras_dm_excluidos (
      sku TEXT PRIMARY KEY,
      excluido_por TEXT,
      excluido_en TIMESTAMPTZ DEFAULT now()
    );
...
// Mismo criterio que compras_dm_excluidos, pero para la tabla "Recomendación
// de compra con presupuesto" -- pedido del usuario: un producto eliminado
// ahí no debe volver a aparecer aunque se recalcule la recomendación de
// nuevo. Tabla propia porque es otra lista distinta (esta compite por
// presupuesto contra el catálogo completo, no solo DM).
export async function asegurarTablaRecomendacionCompraExcluidos(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS recomendacion_compra_excluidos (
      sku TEXT PRIMARY KEY,
      excluido_por TEXT,
      excluido_en TIMESTAMPTZ DEFAULT now()
    );
...
// Mismo criterio que compras_dm_excluidos, pero para el módulo "Compras
// Intcomex" -- pedido del usuario. A diferencia de DM, el proveedor
// "Intcomex" ya se identifica en bsale-sku-report.js (texto de recepción
// que calza /intcomex/i), así que este módulo no necesita su propio
// regex/whitelist de categorías: solo filtra por s.proveedor === 'Intcomex'.
export async function asegurarTablaComprasIntcomexExcluidos(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS compras_intcomex_excluidos (
      sku TEXT PRIMARY KEY,
      excluido_por TEXT,
      excluido_en TIMESTAMPTZ DEFAULT now()
    );
...
// Módulo "Comparativa de proveedores" en Compras (Coimco vs Laptop Center,
// pensado desde el diseño para N proveedores) -- pedido del usuario: hoy
// manda el mismo pedido a ambos proveedores a mano y decide a quién
// comprarle cada línea según precio neto + stock. Ver comparadorProveedores
// en lib/comparadorProveedores.js para la lógica de comparación/equivalencia;
// acá solo el esquema.
//
// No hay tabla PRODUCTS propia: el catálogo interno ya vive en Bsale
// (datosSkus en compras.html, vía bsale-sku-report) -- comparador_solicitud_items
// referencia el SKU cuando existe, pero además guarda nombre/especificaciones
// en texto porque un pedido puede incluir specs (voltaje, conector, modelo de
// panel) que Bsale no registra.
//
// Tampoco hay tabla de historial de precios aparte: comparador_cotizacion_items
// ya queda con fecha (vía comparador_cotizaciones.fecha) por cada vez que se
// carga una cotización, así que el historial por SKU es una consulta sobre
// esa misma tabla, no una tabla nueva.
export async function asegurarTablaComparadorCompras(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS comparador_proveedores (
      id SERIAL PRIMARY KEY,
  ...
  `;
  // Semilla inicial -- el diseño soporta N proveedores (ver 15 en el pedido
  // del usuario), pero hoy solo se usan estos dos.
  await sql`INSERT INTO comparador_proveedores (nombre) VALUES ('Coimco'), ('Laptop Center') ON CONFLICT (nombre) DO NOTHING;`;

  await sql`
    CREATE TABLE IF NOT EXISTS comparador_solicitudes (
      id SERIAL PRIMARY KEY,
      nombre TEXT,
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS comparador_solicitud_items (
      id SERIAL PRIMARY KEY,
      solicitud_id INTEGER NOT NULL REFERENCES comparador_solicitudes(id) ON DELETE CASCADE,
      sku TEXT,
  ...
  // origen: 'manual' | 'imagen' | 'excel'. imagen_origen guarda la foto tal
  // cual se subió (data URL, mismo patrón que reportes_error.imagen) -- solo
  // para poder auditar de dónde salió un precio si algo se leyó mal.
  await sql`
    CREATE TABLE IF NOT EXISTS comparador_cotizaciones (
      id SERIAL PRIMARY KEY,
      solicitud_id INTEGER NOT NULL REFERENCES comparador_solicitudes(id) ON DELETE CASCADE,
      proveedor_id INTEGER NOT NULL REFERENCES comparador_proveedores(id),
  ...
  // estado_stock: 'disponible' | 'sin_stock' | 'parcial' | 'por_confirmar'.
  // confianza_equivalencia se calcula al comparar (ver comparadorProveedores)
  // y se guarda acá para no tener que recalcularla cada vez que se lista.
  await sql`
    CREATE TABLE IF NOT EXISTS comparador_cotizacion_items (
      id SERIAL PRIMARY KEY,
      cotizacion_id INTEGER NOT NULL REFERENCES comparador_cotizaciones(id) ON DELETE CASCADE,
      solicitud_item_id INTEGER REFERENCES comparador_solicitud_items(id),
      codigo_proveedor TEXT,
      descripcion TEXT,
      cantidad_disponible NUMERIC,
      estado_stock TEXT NOT NULL DEFAULT 'por_confirmar',
      precio_neto NUMERIC,
      dias_entrega INTEGER,
      observaciones TEXT,
      confianza_equivalencia TEXT,
      created_at TIMESTAMPTZ DEFAULT now()
    );
  `;

  // Solo existe una fila acá cuando una persona CAMBIÓ manualmente el
  // proveedor recomendado por el sistema (punto 17 del pedido) -- si no hay
  // fila, se usa la recomendación automática calculada en vivo. Así el
  // override queda explícito y separado del cálculo, y nunca queda
  // "perdido" dentro de un total recalculado.
  await sql`
    CREATE TABLE IF NOT EXISTS comparador_decisiones (
      id SERIAL PRIMARY KEY,
      solicitud_item_id INTEGER NOT NULL UNIQUE REFERENCES comparador_solicitud_items(id) ON DELETE CASCADE,
      proveedor_seleccionado_id INTEGER NOT NULL REFERENCES comparador_proveedores(id),
  ...
  // Memoria permanente de equivalencias confirmadas a mano ("el código X de
  // Laptop Center ES este producto") -- una vez confirmada, no hace falta
  // volver a adivinar la equivalencia la próxima vez que llegue una
  // cotización con el mismo código de ese proveedor.
  await sql`
    CREATE TABLE IF NOT EXISTS comparador_sku_mapa (
      id SERIAL PRIMARY KEY,
      proveedor_id INTEGER NOT NULL REFERENCES comparador_proveedores(id),
      codigo_proveedor TEXT NOT NULL,
      solicitud_item_nombre TEXT NOT NULL,
      confirmado_por TEXT,
      created_at TIMESTAMPTZ DEFAULT now(),
      UNIQUE(proveedor_id, codigo_proveedor, solicitud_item_nombre)
    );
  `;

  // Fila única (mismo patrón que saldo_bci) -- "umbral de ahorro mínimo"
  // (punto 7 del pedido): si el ahorro del proveedor más barato es menor a
  // esto, se marca "revisar conveniencia" en vez de recomendarlo directo.
  await sql`
    CREATE TABLE IF NOT EXISTS comparador_config (
      id INTEGER PRIMARY KEY DEFAULT 1,
      umbral_ahorro_minimo NUMERIC NOT NULL DEFAULT 1000,
      actualizado_en TIMESTAMPTZ DEFAULT now()
    );
  `;
  await sql`INSERT INTO comparador_config (id) VALUES (1) ON CONFLICT (id) DO NOTHING;`;
}

// ---------- Productos en tránsito (envíos desde proveedores, ej. China) ----------
// Carga 100% manual (pedido del usuario) -- no hay integración con
// CargoWise ni ningún otro sistema de tracking; alguien revisa el estado
// del envío ahí y lo transcribe acá. "productos" queda como JSONB (no una
// tabla aparte) porque es el detalle de la carga (SKU/nombre/cantidad),
// prácticamente fijo una vez creado el envío -- no se edita línea por
// línea como sí ocurre con comparador_solicitud_items. "eventos" SÍ es
// tabla aparte porque se van agregando de a uno a medida que avanza el
// envío (mismo patrón que el historial de seguimiento de una naviera).
export async function asegurarTablaProductosTransito(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS productos_transito_envios (
      id SERIAL PRIMARY KEY,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS productos_transito_eventos (
      id SERIAL PRIMARY KEY,
      envio_id INTEGER NOT NULL REFERENCES productos_transito_envios(id) ON DELETE CASCADE,
...
// Preferencias de vista por usuario (filtros por defecto, columnas
// visibles, búsquedas guardadas, etc.), pensadas para que cada página las
// use como quiera: "pagina" identifica el HTML (ej. "compras") y "clave"
// identifica QUÉ parte de esa página es (ej. "catalogo-completo"), para
// poder guardar varias preferencias independientes dentro de una misma
// página sin que se pisen entre sí. "valor" es JSONB de forma libre --
// cada página define su propia forma (ej. {categoria, busqueda, urgencia})
// y la lee con un default, así un cambio de forma futuro no rompe lo ya
// guardado (los campos que no vengan simplemente quedan en su default).
export async function asegurarTablaPreferenciasUsuario(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS preferencias_usuario (
      ...
      actualizado_en TIMESTAMPTZ DEFAULT now(),
      PRIMARY KEY (usuario_email, pagina, clave)
    );
  `;
}

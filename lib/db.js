...
export async function asegurarTablaShopifyAgotados(sql) {
  ...
  `;
  // SKU de la primera variante del producto -- se usa para cruzar contra
  // Bsale y detectar productos agotados que siguen publicados en Shopify
  // pero cuyo SKU ya no existe o está deshabilitado en Bsale (ver
  // manejarAgotados en api/shopify-report.js).
  await sql`ALTER TABLE shopify_agotados ADD COLUMN IF NOT EXISTS sku TEXT;`;
}
